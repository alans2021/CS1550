...
struct my_queue{
	//data
	struct task_struct * task; //Like 'proc()' in Lab 1
	//pointers
...
asmlinkage long sys_cs1550_down(struct cs1550_sem *sem){ //asmlinkage means always fetch parameter from stack, not register
	...
	if (sem->value < 0){
		...
		if(sem->head == NULL){
			...
			struct my_queue * previous = sem->tail;
			previous->next = queue;
			sem->tail = queue;
		}
...
asmlinkage long sys_cs1550_up(struct cs1550_sem *sem){
	...
	if (sem->value <= 0){
		...
		remove_queue = sem->head;
		if(remove_queue == NULL)
			return 0;
		
		sleeping_task = remove_queue->task;
		...
		//Wakeup();
		spin_unlock(&sem_lock);
		wake_up_process(sleeping_task); //sleeping_task is the first entry in queue
	}
